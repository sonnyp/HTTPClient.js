(function (global) {
  ...
  var formatQuery = function (query, sep, eq) {
    ...
    if (typeof query === 'object') {
      for (var i in query) querystring += i + eq + query[i] + sep
  ...
  var BrowserRequest = function (opts) {
    ...
    if (typeof opts.jsonp === 'string') {
      if (opts.body) utils.warn('Request body ignored for JSONP')

      jsonp(opts, function (err, body) {
        if (err) request.onerror(err)
        else response.onend(body)
      })
      return
    }
